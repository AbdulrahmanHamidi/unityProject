  ...
  void Update() {
    if (_material != null) {
      ...
      if (_intObj.isPrimaryHovered && usePrimaryHover) {
        ...
        //EDITED CODE
        if (name == "Cube")    //We need to be sure  if the object is our box or not because hand can touch anything
          ManagerScript.Instance.MakeItLastTouched(transform.parent.name);
      }
      else {
